...
describe('casos de uso de vehículos', () => {
  it('registra y lista vehículos con actor temporal', async () => {
    ...
    const proveedorIdentidad = new ProveedorIdentidadTemporal();
  ...
  it('rechaza matrícula duplicada de forma global aunque el vehículo anterior esté inactivo', async () => {
    const repositorioVehiculos = new RepositorioVehiculosEnMemoria();
    const proveedorIdentidad = new ProveedorIdentidadTemporal();
  ...
  it('desactiva un vehículo sin borrar sus eventos', async () => {
    ...
    const proveedorIdentidad = new ProveedorIdentidadTemporal();
    await registrarVehiculo({ repositorioVehiculos, proveedorIdentidad }, entradaVehiculo());
    await repositorioEventos.guardar(crearEventoVehiculo({
      ...entradaEvento(),
    ...
    const vehiculo = await repositorioVehiculos.buscarPorId(crearIdentificador('vehiculo-1234 ABC'));
    const eventos = await repositorioEventos.listarPorVehiculo(crearIdentificador('vehiculo-1234 ABC'));
  ...
  it('registra evento y actualiza kilometraje cuando el evento es más reciente', async () => {
    ...
    const vehiculo = await repositorioVehiculos.buscarPorId(crearIdentificador('vehiculo-1234 ABC'));
    const eventos = await unidadTrabajo.listarPorVehiculo(crearIdentificador('vehiculo-1234 ABC'));
