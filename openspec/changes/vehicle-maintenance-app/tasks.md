...
- [x] GREEN: implementar `OperacionesBootstrapPostgres` contra Postgres administrativo real (server-only, no dobles) + añadir `mv_households_nombre_key` (`unique (nombre)`) mediante la migración `20260711000000_mv_households_nombre_unique.sql`. Verificado contra Supabase local: el constraint existe y rechaza duplicados; el bootstrap real crea/reutiliza usuario, hogar y membresía `admin` de forma idempotente.
