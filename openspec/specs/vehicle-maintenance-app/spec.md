...
- Los datos MUST estar aislados por hogar (multi-tenant): vehículos y eventos pertenecen a un hogar y no MUST ser visibles ni modificables desde otro hogar. La matrícula MUST ser única por hogar (`unique (household_id, matricula)`), incluyendo vehículos inactivos.
...
## Decisiones de especificación (resueltas durante la implementación)

- Stack: Next.js + TypeScript + Tailwind + Zod + Vitest (ver `openspec/changes/archive/2026-07-11-vehicle-maintenance-app/diseno.md`, §13).
- Coste de evento: opcional; el sistema no exige coste obligatorio para registrar un mantenimiento o avería.
- Matriz detallada de permisos por rol: sigue pendiente de definición de producto; `admin`/`editor` existen como concepto de dominio pero sin autorización real aplicada en el MVP.

## Estado

Implementado y verificado. Este documento es la fuente de verdad vigente para el dominio `vehicle-maintenance-app`; el historial completo de propuesta, diseño, tareas y verificación del corte inicial vive en `openspec/changes/archive/2026-07-11-vehicle-maintenance-app/`.
